...
func TestExtensionObservationContractsRequireDetachedIdentityConsistentFacts(t *testing.T) {
	now := time.Now().UTC()
	actor := agent.ActorIdentity{Kind: agent.ActorService, ID: "runtime"}
	diagnostic := session.ExtensionDiagnostic{
		InvocationID: "invocation-1", Sequence: 1,
		Descriptor: hook.ExtensionDescriptor{Key: "fixture", DefinitionDigest: "sha256:" + strings.Repeat("a", 64)},
		Boundary:   hook.BoundaryToolResult, SessionID: "session-1", RunID: "run-1", StepID: "step-1", ToolCallID: "call-1",
		Status: hook.InvocationFailed, ErrorCode: "hook_failed", Effect: hook.EffectApplied, Context: hook.ContextNone,
	}
	validAudit := observe.AuditRecord{
	...
	if err := validAudit.Validate(); err != nil {
		t.Fatalf("valid extension audit rejected: %v", err)
	}
	wrongIdentity := validAudit
	wrongIdentity.Identity.RunID = "other-run"
	if err := wrongIdentity.Validate(); err == nil {
		t.Fatal("extension audit with a mismatched Run identity was accepted")
	}
	wrongStatus := validAudit
	wrongStatus.Decision = string(hook.InvocationSucceeded)
	if err := wrongStatus.Validate(); err == nil {
		t.Fatal("extension audit with a mismatched status was accepted")
	}
	foreignPayload := observe.AuditRecord{
		Kind: observe.AuditModelConfigChanged, At: now,
		Identity: observe.Identity{SessionID: "session-1", Actor: actor},
		Action:   "provider/model", Decision: "committed", Extension: &diagnostic,
	}
	if err := foreignPayload.Validate(); err == nil {
		t.Fatal("non-extension audit carrying extension data was accepted")
	}
	for _, name := range []string{observe.MetricExtensionJournalEntries, observe.MetricExtensionJournalBytes} {
		metric := observe.MetricRecord{
			Name: name, Kind: observe.MetricGauge, Value: 12, At: now,
			Identity: observe.Identity{SessionID: "session-1", Actor: actor},
		}
		if err := metric.Validate(); err != nil {
			t.Fatalf("valid extension gauge %q rejected: %v", name, err)
		}
		metric.Kind = observe.MetricCounter
		if err := metric.Validate(); err == nil {
			t.Fatalf("extension metric %q accepted a non-gauge kind", name)
		}
	}
}
