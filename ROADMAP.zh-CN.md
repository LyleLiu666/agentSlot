...
AgentSlot 在 `v1.0.0` 前允许发布两类版本，不能把二者混为一谈：

- **试用验证版本**：供真实 Agent 项目引入、发现接口问题和积累可移植性证据。
  它必须来自已经推送并通过 CI 的 `main` 提交，但可以仍有 `Contracted` 而尚未
  `Conformant`/`Proven` 的组件；版本说明必须明确不保证 API 稳定。
- **稳定推荐版本**：表示框架已经取得跨实现、跨项目的一致性证据，可以作为长期
  依赖推荐给外部项目。下面的成熟度条件是这类版本的门槛，不是试用验证版本的开发
  或发布门禁。

每次发布都遵循同一条不可倒置的链路：先把发布提交合入并推送到 `main`，确认该提交
的远端 CI 成功，再在这个确切提交上创建 annotated tag 并推送 tag。tag 一经发布不可
移动或复用。

`v0.0.2` 计划作为第一个完整 Agent 框架链路的**试用验证版本**。在正式创建 tag 前，
候选提交用于真实项目试用，
不宣称标准组件生态已经成熟，也不改变以下稳定推荐版本门槛：
...
达到门槛只代表可以讨论稳定推荐版本，不代表自动发布。发布前仍要审查接口是否
足够小、命名是否稳定，以及真实应用是否真的减少了重复劳动。
