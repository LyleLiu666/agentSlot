...
var standardCatalog = Catalog{StandardVersion: StandardVersion, Components: []Component{
	...
	entry("model", "model.executor", "ModelExecutor", KindOne, MaturityContracted, module+"/model", true, profile(1), "globally required", "Validates selected-model capabilities, executes one logical model call, contains retries and continuation, reports post-call usage, and durably records each physical attempt plus an optional bounded adapter-sanitized displayable failure message through the restricted AttemptRecorder.", "全局必需", "校验所选模型能力、执行一次逻辑模型调用、封装重试和续传、报告调用后 Usage，并通过受限 AttemptRecorder 持久记录每次真实请求及可选、受限且由适配器清洗后允许展示的失败消息。"),
	...
	withEvidence(entry("context", "session.store", "SessionStore", KindOne, MaturityConformant, module+"/session", true, profile(1), "globally required", "Persists the whole Session aggregate and atomic revision/CAS transactions; preserves tool arguments by JSON value semantics across encoding and reopen; lists resumable Sessions through bounded, deterministic, lifecycle-scoped cursor pages within an Agent/Workspace scope; History remains the unique append-only fact view.", "全局必需", "持久化完整 Session 聚合及其 revision/CAS 原子事务；跨编码与重开按 JSON 值语义保持工具参数；按 Agent/Workspace 提供有界、确定性排序且绑定 Store 生命周期的游标分页；History 是唯一、append-only 的事实视图。"), Evidence{
