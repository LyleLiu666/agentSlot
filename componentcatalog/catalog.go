...
var standardCatalog = Catalog{StandardVersion: StandardVersion, Components: []Component{
	...
	entry("runtime", "hook.completion_gate", "CompletionGate", KindChain, MaturityContracted, module+"/hook", true, nil, "optional", "Makes a durable ordered complete-or-continue decision at natural Run completion; continuation context targets one preallocated next Step while user steer, cancellation, Goal versioning, and existing Run budgets remain Runtime-owned.", "可选", "在 Run 自然完成边界给出持久、有序的完成或继续决定；continuation context 只进入预分配的下一 Step，用户 steer、取消、Goal 版本和既有 Run 预算仍由 Runtime 独占。"),
	...
	entry("operations", "audit.sink", "AuditSink", KindChain, MaturityContracted, module+"/observe", true, nil, "optional", "Receives model-config, tool-policy, and payload-free extension-transition facts without message content, tool arguments, or extension context.", "可选", "接收模型配置、工具策略和不含 payload 的扩展状态转换事实，不包含消息内容、工具参数或扩展上下文。"),
	...
	entry("operations", "metric.sink", "MetricSink", KindChain, MaturityContracted, module+"/observe", true, nil, "optional", "Receives normalized counters, durations, and gauges—including extension-journal entry/byte gauges—with detached attributes.", "可选", "接收带隔离属性副本的标准化计数、耗时和 gauge，包括 ExtensionJournal 条目数与字节数。"),
