...
`
	...
	text := string(result)
	if strings.Contains(text, "stale") || !strings.Contains(text, "Mapped standard component ecosystems | 45") || !strings.Contains(text, "`model.token-counter`") || !strings.Contains(text, "After.") {
...
`
	...
	if strings.Contains(string(result), "stale") || !strings.Contains(string(result), "| 已映射的标准组件生态位 | 45 |") {
