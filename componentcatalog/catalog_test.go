...
func TestStandardCatalogPreservesApprovedInventory(t *testing.T) {
	...
	if got, want := len(catalog.Components), 45; got != want {
	...
	wantIDs := []string{
		...
		"gateway.channel", "goal.evaluator", "goal.store", "health.contributor", "hook.completion_gate", "hook.input_gate", "hook.tool_preflight", "hook.tool_result",
	...
	if counts.Mapped != 9 || counts.Contracted != 35 || counts.Conformant != 1 || counts.Proven != 0 || counts.Assembled != 0 {
		t.Fatalf("maturity counts = %#v", counts)
	}
	if counts.AtLeastContracted() != 36 {
		t.Fatalf("at-least-contracted count = %d, want 36", counts.AtLeastContracted())
	}
