...
| Slot ID | 契约 | 类型 | Profile 规则 | 职责 | 成熟度 |
| --- | --- | --- | --- | --- | --- |
| `model.executor` | `ModelExecutor` | `One` | 全局必需 | 校验所选模型能力，以受校验的临时输出/reset/终态流执行一次逻辑模型调用，封装重试和续传、报告调用后 Usage，并通过受限 AttemptRecorder 持久记录每次真实请求及可选、受限且由适配器清洗后允许展示的失败消息。 | 已通过一致性验证 |
