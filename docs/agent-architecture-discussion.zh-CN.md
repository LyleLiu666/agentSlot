...
固定 Gateway 和固定 SessionManager 使用注册表，但注册表的生命周期所有者始终是
启动后的 Application Runtime。
...
- **否决的方案及原因：** 每个 Run 创建临时 Loop 会反复重建命令、取消和配置边界；对只读浏览也创建 Runtime 会把查询行为变成资源占用；让 Gateway、Manager、Channel 或全局变量分别持有注册表会使生命周期和关闭责任分裂；把单进程边界描述为第一版临时妥协会留下隐式分支。
- **对接口、存储、Gateway 和实现的影响：** `Runtime.Stop` 负责关闭并清空 Registry。`Close` 只移除内存 AgentRuntime，不删除 Session。再次 resume 使用最新 Runtime 固定配置，但恢复该 Session 已持久化的模型配置。跨进程所有权、租约或迁移必须重新评审。
- **状态：** 已确定。

### A-004 SessionManager 固定，SessionStore 可替换

- **问题 / 背景：** Session 创建、恢复、fork、持久化和 Agent 命令具有不同变化原因。
- **最终决定：** SessionManager 是框架固定能力，负责 create/resume/fork/摘要启动；`session.store` 是可替换 `One` Slot，原子持久化完整 Session History、Context、Queue、RunJournal、SessionModelConfig 和 SessionRevision。固定 Runtime 接收执行命令。
- **必须满足的不变量：** Manager 只依赖 Store、Application 默认模型和内部 ID 生成器；不保存第二份真相；Runtime 只接收恢复完成的 Session；任何组件都不能绕过 SessionStore 核心事务。
- **否决的方案及原因：** 可替换 Manager 会让固定 Session 生命周期出现多套语义；Manager 与 Store 合并则会让固定规则和存储技术绑定；把生命周期职责塞进循环会污染执行代码。
- **对接口、存储、Gateway 和实现的影响：** 删除公共 `session.manager` Slot 和接口。Memory/File 模块只贡献 SessionStore；`CreateSession`/`ResumeSession` 是固定 Gateway 的生命周期入口。
- **状态：** 已确定。
...
- **对接口、存储、Gateway 和实现的影响：** 固定 Manager 提供完整 Fork、从合法 HistorySequence/已完成 Step 的检查点 Fork 和摘要启动。Fork 禁止切在未配对 ToolCall 中间，保存父 Session、截止 Sequence 和来源 Fact 身份；Queue、RunJournal 和活跃 Run 不继承；默认继承来源 Session 当前 ModelConfig；来源 usage 不在子 Session 重复计费。摘要启动只提交显式摘要输入。
- **状态：** 已确定。
...
- **最终决定：** 标准 Agent 框架提供一个固定、进程内且不监听网络的 Gateway；每个启动后的 Application Runtime 持有一个实例。所有面向用户或外部调用方的 GatewayChannel 都只能通过 GatewayAccess 操作 Agent。Gateway 不是 Slot，也不要求独立部署。
- **必须满足的不变量：** Gateway 是唯一用户交互后端边界；不拥有 Session 真相，不直接写 SessionStore，不实现模型/工具循环，不复制 AgentRuntime 状态。它只完成调用校验、结构化命令分发、SessionView/历史分页、临时流和 revision 通知。AgentRuntime 不反向依赖具体 Channel。
- **否决的方案及原因：** 让 Channel 直接调用 Runtime 会使每种 UI 重复实现 revision、幂等和错误规则；把 Gateway 当成网络转发服务无法保护 Agent 与 UI 的边界；每 Session 一个 Gateway 会重复状态；让 Gateway 直接承载业务真相会形成超级对象。
- **对接口、存储、Gateway 和实现的影响：** `gateway.channel` 是标准 `Many` Slot且至少安装一个。TUI、Web、CLI、RPC、IM 和函数调用分别实现 Channel；Channel 的 Module 管理 Start/Stop、协议、认证授权和限流，Channel 只能获得 GatewayAccess。
- **状态：** 已确定。
...
- **最终决定：** Gateway 暴露与传输协议无关的结构化 Go 调用、结果、SessionView、历史分页、临时流和 revision 通知。进程内 Channel 直接调用；只有跨进程 Channel 才映射为 RPC，具体 wire protocol 由 Channel 决定。
...
- **对接口、存储、Gateway 和实现的影响：** Gateway 核心合同不出现 HTTP、WebSocket、gRPC、socket 或响应码；删除 `interaction.entrypoint`、`gateway.transport`、`gateway.identity`、`gateway.route` 和 `gateway.delivery` 标准 Slot，承载职责由 GatewayChannel 自己完成。
...
- **对接口、存储、Gateway 和实现的影响：** Gateway 提供 SessionView、历史分页和事件订阅；Cancel 必须是显式且带 ExpectedRevision 的 Gateway 命令。远程 Channel 负责连接重建。
- **状态：** 已确定。
...
### A-015 重连基于 SessionView 和 revision

- **问题 / 背景：** 临时 chunk 不持久化，客户端仍需要在重连后恢复一致界面。
- **最终决定：** chunk/reset 是允许丢失的提交前事件；持久提交后只广播 `SessionID + Revision`。重连或收到通知后读取权威 SessionView；AgentSlot 不保存 chunk 游标、框架级客户端 ACK 或消费游标。
- **必须满足的不变量：** SessionView 只含已提交事实和当前状态；客户端替换本地临时投影；传输回执不能改变 History、Context、Run 或业务完成状态。缓冲满时先丢 chunk；持久 revision 通知不能投递时关闭订阅。
- **否决的方案及原因：** 把每客户端 ACK 或游标放进 SessionStore，会把展示与传输状态变成核心业务状态，并带来客户端身份、租期和无界清理问题。
- **对接口、存储、Gateway 和实现的影响：** SessionView 返回状态、Queue、模型配置和最近 100 个逻辑 Step；更老 History 用 `BeforeHistorySequence` 游标分页，每页最多 100 Step。订阅关闭不取消 Run；具体 Channel 可私有实现可靠投递，但不新增标准 ACK Slot。
- **状态：** 已确定。
...
- **对接口、存储、Gateway 和实现的影响：** SessionStore、SessionView 和 HistoryPage 明确区分持久事实、模型投影与待处理输入。
- **状态：** 已确定。

### A-017 History 严格 append-only

- **问题 / 背景：** History 是模型交互和工具事实的审计来源。
- **最终决定：** 完整 Session History 是唯一事实序列，按真实发生顺序保存 Message、ToolCall、ToolResult、Run、ModelAttempt、ModelConfigChanged、ContextContribution 和 RunBudgetExceeded 等事实，并严格只追加。
- **必须满足的不变量：** 每个 Fact 有稳定 FactID、严格递增 HistorySequence、Session/Run/Step 身份、时间、ActorIdentity 和类型；已提交项不能修改、删除、换位或向前插入；History 不以“可直接发送给 Provider”为合法性标准。
- **否决的方案及原因：** 压缩时改写或删除旧 History 会失去恢复、审计和重新派生 Context 的依据。
- **对接口、存储、Gateway 和实现的影响：** Store 原子推进 SessionRevision 和 HistorySequence；更正只能追加新事实；完整 tool call 一旦产生就立即成为事实。术语统一为“完整 Session History”，不新增 Ledger 概念。
- **状态：** 已确定。

### A-018 Context 是版本化派生视图

- **问题 / 背景：** 模型输入需要压缩，但压缩不应篡改事实。
- **最终决定：** Context 是满足所选模型协议的版本化视图；完整版本包含当时的 SystemPrompt、模型输入、Tool definitions、ModelConfig 和附件投影，不含凭据或网络 Header。Agent 启动配置选择默认 `LatestOnly` 或调试用 `RetainAll`。
- **必须满足的不变量：** 每次模型调用绑定明确 ContextVersion；旧 History 保持不变；未配对 tool call 不进入下一次模型请求。ContextSource 只能为新 Step 提议追加内容，Runtime 必须先持久化 ContextContributionFact，Source 不得改写旧 Step。
- **否决的方案及原因：** 直接裁剪 History 会把模型预算优化变成数据丢失。
- **对接口、存储、Gateway 和实现的影响：** LatestOnly 只保存最新完整 Context；RetainAll 保存每个 Step 的完整逻辑请求。Context 记录来源 HistorySequence、贡献 Fact 和配置版本。
- **状态：** 已确定。
...
### A-021 所有外部写命令使用统一 expected revision/CAS

- **问题 / 背景：** 多个 Channel、用户操作和 AgentRuntime 可能同时修改同一 Session。
- **最终决定：** 每个 Session 只有一个 SessionRevision；Send、Steer、Queue 操作、模型切换、Cancel 和 Close 等全部外部写命令都携带 ExpectedRevision 并通过 CAS 提交。
- **必须满足的不变量：** 旧 revision 一律返回 `RevisionConflictError{CurrentRevision, SnapshotRequired:true}`；不自动重试用户命令；认领后的 Queue 修改同样冲突。
- **否决的方案及原因：** 后写覆盖会误删已被 Runtime 认领的输入或篡改投递顺序。
- **对接口、存储、Gateway 和实现的影响：** Gateway 返回最新 revision；Channel 必须无损传递；客户端冲突后刷新 SessionView。
- **状态：** 已确定。
...
- **对接口、存储、Gateway 和实现的影响：** 恢复事务执行类型迁移；SessionView 明确 held 原因。
- **状态：** 已确定。
...
- **最终决定：** Agent 配置分成三部分：`AgentRuntimeConfig` 固定 `SystemPrompt`、严格 ToolKeys、ContextRetentionMode、Context 配置和可选 MaxTokensPerRun；Agent 默认模型只初始化新 Session；`SessionModelConfig` 保存当前实际 Provider、Model、Reasoning 和模型参数。
...
- **必须满足的不变量：** 临时 chunk 不进入 History；物理 Attempt 事实进入 History 但不进入模型 Context；已提交完整消息不回滚；最终失败结束当前 Run，使 Session 回到 `idle`，且不自动消费旧 Queue。
- **否决的方案及原因：** Runtime 猜测 Provider 恢复方式会把供应商协议污染进状态机；把半截持久化为 assistant message 会制造伪事实。
- **对接口、存储、Gateway 和实现的影响：** 流事件带 AttemptID；Gateway 能撤销对应临时展示；崩溃后未配对 started 追加 `outcome_unknown` terminal。
- **状态：** 已确定。
...
### A-044 Hook 只保留执行前收尾提议，提交观察独立

- **问题 / 背景：** 可扩展收尾行为需要 Hook，但任意阶段会让主循环不可推理。
- **最终决定：** `agent.hook` 只保留 `BeforeRunComplete`，在完整 assistant 消息已保存、Run 尚未终结时返回受控 follow-on proposal。提交后观察拆为 `session.commit.observer: Chain`。
- **必须满足的不变量：** Hook 不能直接修改 Queue、History、Context 或 Run 状态，也不能自行启动 Step；Runtime 拥有唯一继续权。CommitObserver 只异步接收 SessionID、Revision 和 FactSequence 范围；同 Session 按 revision 顺序执行，错误和 panic 不回滚提交。
- **否决的方案及原因：** 万能 Hook 会制造第二个循环控制者；把执行提议和提交观察放在同一接口会混淆权限；宽泛 `runtime.observer` 与专用 Trace/Metric/Audit/Usage 重叠。
- **对接口、存储、Gateway 和实现的影响：** 删除 AfterCommit Hook 和 `runtime.observer`；新增 `session.commit.observer`。Trace、Metric、Audit、Usage 保持专用 Slot。
- **状态：** 已确定。
...
- **必须满足的不变量：** 先持久化再异步通知 CommitObserver；核心提交失败时不得宣称动作成功。
...
- **最终决定：** Build 解析标准 typed Slot 并形成不可变 Assembly，不创建注册表。`Application.Start` 创建应用级 Runtime，同时创建并持有唯一 RuntimeRegistry、固定 SessionManager 和固定 Gateway；所有 GatewayChannel 只绑定同一个 GatewayAccess。
- **必须满足的不变量：** 用户继续只使用统一 Build/Start/Run/Stop；运行时不得扫描包、查询全局容器或保留 Build Resolver；初始化失败不得登记半成品 Runtime。停止时先阻止 Channel 接收命令，再收束 AgentRuntime，最后逆序停止共享组件。
- **否决的方案及原因：** 每个 Channel 自建注册表会产生重复 Runtime；向 Channel 暴露 RuntimeAccess 会绕过 Gateway；公开 RuntimeFactory 会把固定循环伪装成替换点；运行时任意查询 Assembly 会形成服务定位器。
- **对接口、存储、Gateway 和实现的影响：** Gateway 不接收或返回 Runtime 指针，对 Channel 只返回稳定 ID、revision、View、命令结果和事件。包内私有装配锚点不进入 Profile、组件地图或成熟度计分。
- **状态：** 已确定。

### A-047 GatewayChannel 统一接入，交互命令只向固定 Gateway 注册

- **问题 / 背景：** TUI、Web、CLI、IM 和函数调用必须共享后端；`/model`、`/help` 等命令又要支持多种 UI，不能各自实现一套语义。
...
- **必须满足的不变量：** 根包不导入 Agent、Provider、UI、存储或 Gateway；GatewayChannel
...
- **最终决定：** SessionManager 是框架固定能力，不是 Slot；SessionStore 是标准
  `One` Slot。Manager 只通过 Store 创建、恢复和派生 Session。
- **必须满足的不变量：** 固定规则不保存第二份 Session 真相；存储实现不得改写
  Manager 的生命周期语义。
- **状态：** 已确定，见 A-004、A-007。

### D-002 临时模型 chunk 永不持久化

- **最终决定：** chunk/reset 只用于提交前显示；完整消息和结构化 Attempt 才是事实。
- **必须满足的不变量：** 不保存逐 chunk 游标或 ACK；半流失败不产生半条 assistant
  Message。
- **状态：** 已确定，见 A-013、A-015、A-036。

### P-001 完整 Session History 是唯一事实来源

- **最终决定：** 不新增 Ledger 名称。完整 Session History 以类型化、append-only Fact
  保存业务事实、模型物理 Attempt、用量、配置变更、Context 贡献和预算终态；不同用途
  从它形成 View、Context、计费和恢复投影。
- **状态：** 已确定，见 A-017、A-026、A-035。

### P-002 一个 SessionRevision 严格保护所有外部写

- **最终决定：** 不拆多个外部 revision。所有外部写命令使用同一个 ExpectedRevision；
  旧值返回要求刷新 View 的结构化冲突，不自动重试用户动作。HistorySequence、
  ContextVersion 等仍各自表达排序或身份，不承担外部 CAS。
- **状态：** 已确定，见 A-021。

### P-003 ContextSource 保留但严格收窄

- **最终决定：** Source 只能为当前新 Step 提议追加内容；Runtime 先追加
  ContextContributionFact，再装配 Context。Source 不得替换、删除或插入旧 Step。
- **状态：** 已确定，见 A-018。

### P-004 ToolKeys 是显式严格白名单

- **最终决定：** nil、空列表和未配置均表示无工具；空、未知或重复 key 在 Build
  阶段失败。安装 Tool Module 不等于自动授权全部工具。
- **状态：** 已确定。

### P-005 Gateway 只流临时输出和持久 revision 通知

- **最终决定：** chunk/reset 可丢；提交后仅通知 SessionID+Revision，UI 再读
  SessionView。持久通知不能投递时关闭订阅，而不是无界缓存。
- **状态：** 已确定，见 A-015。

### P-006 ActorIdentity 是事实来源，不是认证凭据

- **最终决定：** 外部命令和 HistoryFact 记录 local_user、remote_user、service 或 agent
  来源。Channel 负责远程认证授权；Gateway 不用 ActorIdentity 重新认证，也不保存凭据。
- **状态：** 已确定。

### P-007 Hook 与提交观察拆分

- **最终决定：** AgentHook 只留 BeforeRunComplete；`session.commit.observer` 异步观察
  已提交 FactSequence 范围；删除 AfterCommit Hook 和 `runtime.observer`。
- **状态：** 已确定，见 A-044、A-045。

### P-008 ContextVersion 保存可解释的逻辑请求

- **最终决定：** ContextVersion 关联来源 HistorySequence、ContextContributionFact、
  Runtime/模型配置和完整逻辑请求；LatestOnly 只保留最新版本，RetainAll 保留全部 Step。
- **状态：** 已确定，见 A-018、A-033。

### P-009 只标准化每 Run Token Budget

- **最终决定：** 只提供可选 MaxTokensPerRun，0 表示无限；达到预算追加
  RunBudgetExceeded 并回到 idle。Queue 不设标准容量上限；不定义 MaxStep、
  MaxToolCall 或 MaxRunDuration。
- **状态：** 已确定。
...
- **讨论结论：** 接受风险判断，并收窄 Hook 权限。`BeforeRunComplete` 只能在完整 assistant 消息提交后返回 follow-on proposal；固定 Runtime 拥有唯一继续权。提交观察由独立 SessionCommitObserver 完成。
...
- **讨论结论：** 不接受把 ACK 提升为 AgentSlot 标准。框架既不保存临时 chunk 游标，也不保存客户端对持久事实的 ACK；重连使用客户端 revision 与 SessionView。可靠投递可由具体 Channel 或外部消息系统私有实现。
...
- **讨论结论：** 接受 Provider-specific recovery 不属于 Runtime。AgentRuntime 发起一次逻辑调用；ModelExecutor 自行管理物理尝试。每次真实请求有 AttemptID，started/terminal 和用量进入完整 Session History，但不进入模型 Context。
- **对既有决策的处理：** 已修订 A-035、A-036；具体重试次数和退避由 Executor 配置并测试，不成为标准 Profile 语义。
- **状态：** 已讨论并接受修改。
...
- 进程内 Gateway 可以通过私有访问面操作 Runtime；Channel 只能取得稳定 ID、revision、
  SessionView、命令结果和事件；包内私有装配 Slot 不属于公共扩展边界；
...
| 事项 | 当前边界 | 需要的决定或证据 |
| --- | --- | --- |
| GatewayChannel 协议 | 固定 Gateway 核心与承载无关；Channel 必须通过 GatewayAccess | 各 Channel 自行定义 HTTP/SSE、WebSocket、gRPC、ACP 等协议，并用合同测试证明语义一致 |
| Queue 容量、背压和配额 | 标准架构不设 Queue 数量上限 | 产品或 Channel 可施加私有限流，但不能改变已接受消息的持久化语义 |
| 模型重试 | Executor 负责并逐 Attempt 持久化 | 次数和退避属于 Executor 配置；标准 Run 只提供 MaxTokensPerRun |
